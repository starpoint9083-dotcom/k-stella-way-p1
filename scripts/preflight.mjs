...
const requiredSource = [
  ...
  "assertUsageWithinLimit(env,'image_generations','daily_image_limit',60)","SET status='waiting',last_error=''","status='cancelled'","'queue_resume'",'취소된 부족 장면 큐 복구'
];
...
function charContext(label, needle, before=300, after=7000){
  const idx=source.indexOf(needle);
  console.log(`=== CHAR DIAG ${label} idx=${idx} ===`);
  if(idx<0)return;
  console.log(source.slice(Math.max(0,idx-before),Math.min(source.length,idx+after)));
}
charContext('ensureProjectImages','async function ensureProjectImages');
charContext('processProductionItem','async function processProductionItem');
charContext('startProduction','async function startProduction');
charContext('queue-fetch-browser',"api('/api/queue");
charContext('video-plan-browser',"api('/api/video-plan");
