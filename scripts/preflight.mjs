...
const requiredSource = [
  ...
  "['workflow_dispatch','schedule','push']","payload?.runner_environment!=='github-hosted'","payload?.sub!==P3_GITHUB_IMMUTABLE_SUB",
  "return verifySession(env,cookieValue(request,'kstella_session'))","new Error('편성 AI 90초 제한 초과')",'90000',
  "new Error('이미지 AI 180초 제한 초과')",'180000',"new Error('TTS AI 120초 제한 초과')",'120000',
  ...
  "assertUsageWithinLimit(env,'image_generations','daily_image_limit',60)","SET status='waiting',last_error=''","generated_asset_id IS NULL",
  "status IN ('cancelled','generating')","'queue_resume'",'중단된 부족 장면 큐 복구',"recoverable_statuses:['cancelled','generating']",
  'async function rebuildMissingGenerationQueue','LEFT JOIN generation_queue q ON q.scene_id=s.id','s.missing=1 AND q.id IS NULL',
  "SELECT id FROM assets WHERE id=? AND status='active'","UPDATE scenes SET missing=0 WHERE id=?","'scene_state_reconciled'",'기존 active 자산 장면 상태 복구',
  "WHERE NOT EXISTS (SELECT 1 FROM generation_queue WHERE scene_id=?)","INSERT INTO generation_queue(project_id,scene_id,scene_no,requirement_json,prompt,status) SELECT",
  "'queue_rebuilt'",'누락된 부족 장면 큐 재생성','queue_rebuilt:recovery.rebuilt','scene_reconciled:recovery.reconciled',
  "/api/maintenance/recover-lineup-queues","'lineup_queue_recovery'",'편성표 부족 장면 큐 명시 복구','project_count:projects.length,rebuilt,reconciled,projects'
];
...
console.log('=== TTS / AUDIO BINARY DIAGNOSTIC (READ ONLY) ===');
for (const needle of ['aiBinaryResult','generateNarration','narration_assets','returnRawResponse','melotts','@cf/myshell-ai/melotts','mime_type','audio/mpeg','audio/wav','TTS AI 120초 제한 초과','/narration/']) {
  let from=0,shown=0;
  while(shown<5){
    const idx=source.indexOf(needle,from); if(idx<0)break;
    console.log(`--- ${needle} @ ${idx} occurrence=${shown+1} ---`);
    console.log(source.slice(Math.max(0,idx-4500),Math.min(source.length,idx+needle.length+7500)));
